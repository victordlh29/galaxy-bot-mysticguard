...
async function handleCommand(interaction) {
  ...
  try {
    ...
    if (musicControlCmds.includes(commandName) && Array.isArray(config.music.ignoreChannels) && config.music.ignoreChannels.includes(interaction.channelId)) {
      ...
      const denied = await safeEdit({ content: `Los comandos de música están desactivados en este canal. ${hint}`, fetchReply: true }).catch(() => null);
      if (denied) setTimeout(() => denied.delete().catch(() => {}), 5000);
      return;
    }
    if (musicControlCmds.includes(commandName) && !canControlMusic(member, config)) {
      return safeEdit('Solo el rol de control de música (sonidistas) o un administrador puede usar este comando.');
    }
    switch (commandName) {
      case 'play': {
        if (!config.music.enabled) return safeEdit('La música está desactivada en este servidor.');
        if (config.music.commandChannelId && interaction.channelId !== config.music.commandChannelId) {
          return safeEdit('Usa el canal de música configurado para este comando.');
        }
        if (!member.voice.channel) return safeEdit('Entra a un canal de voz primero.');
        const query = interaction.options.getString('query');
        const tracks = await player.resolveQuery(query);
        if (!tracks.length) return safeEdit('No encontré resultados para esa búsqueda.');
      ...
      case 'stop': {
        ...
        return safeEdit('⏹️ Música detenida y salí del canal.');
      }
      case 'pause':
        return safeEdit(player.pause(guild.id) ? '⏸️ Pausada.' : 'Nada reproduciéndose.');
      case 'resume':
        return safeEdit(player.resume(guild.id) ? '▶️ Reanudada.' : 'Nada pausado.');
      case 'volume': {
        ...
        return safeEdit(`🔊 Volumen ajustado a **${vol}%**.`);
      }
      case 'queue': {
        const q = player.getQueue(guild.id);
        if (!q.tracks.length && !q.current) return safeEdit('La cola está vacía.');
        const lines = [];
        ...
        return safeEdit({ embeds: [new EmbedBuilder().setColor('#23A55A').setTitle('Cola de reproducción').setDescription(lines.join('\n'))] });
      }
      case 'nowplaying': {
        const now = player.nowPlaying(guild.id);
        return safeEdit(now ? `🎵 **${now.title}**` : 'No hay nada reproduciéndose.');
      }
      case 'loop': {
        const looping = player.toggleLoop(guild.id);
        return safeEdit(looping ? '🔁 Bucle activado.' : '🔂 Bucle desactivado.');
      }
      case 'autoplay': {
        const auto = player.toggleAutoplay(guild.id);
        return safeEdit(auto ? '📻 Autoplay activado (reproducirá canciones relacionadas al terminar).' : '📻 Autoplay desactivado.');
      }
      case 'filter': {
        const f = interaction.options.getString('filtro');
        player.setFilter(guild.id, f);
        return safeEdit(f === 'off' ? '🎚️ Filtros apagados.' : `🎚️ Filtro **${f}** aplicado.`);
      }
      case 'level': {
      ...
      case 'leaderboard': {
        ...
        if (!top.length) return safeEdit('Todavía no hay XP registrada.');
        const lines = top.map((m, i) => `\`${i + 1}.\` <@${m.userId}> — **${m.xp} XP** (nivel ${m.level})`);
        return safeEdit({ embeds: [new EmbedBuilder().setColor('#F2C94C').setTitle('🏆 Top 10 del servidor').setDescription(lines.join('\n'))] });
      }
      case 'activity': {
      ...
      case 'setup': {
        ...
        if (!isAdminUser) return safeEdit('Solo el dueño o un administrador puede ejecutar este comando.');
        const channel = interaction.channel;
        if (!channel || !channel.isTextBased()) return safeEdit('Ejecuta esto en un canal de texto.');
      ...
      case 'admin': {
        ...
        if (!canConfigure) {
          return safeEdit('Solo el **dueño del servidor** puede configurar los roles de administrador.');
        }
        if (sub === 'add' || sub === 'remove') {
          const role = interaction.options.getRole('rol');
          if (!role) return safeEdit('Rol inválido.');
          const current = Array.isArray(config.adminRoles) ? config.adminRoles : [];
          const set = new Set(current);
          if (sub === 'add') {
            if (set.has(role.id)) return safeEdit(`El rol **${role.name}** ya es administrador.`);
            set.add(role.id);
          } else {
            if (!set.has(role.id)) return safeEdit(`El rol **${role.name}** no está en la lista de administradores.`);
            set.delete(role.id);
          }
      ...
      default:
        return safeEdit('Comando desconocido.');
    }
  } catch (err) {
    console.error(`[CMD] error en ${commandName}:`, err.message);
    return safeEdit('Ocurrió un error ejecutando el comando.').catch(() => {});
  }
}
