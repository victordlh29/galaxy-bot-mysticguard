...
1b. **Redesplegar justrunmy con la versión yt-dlp restaurada (21/08)** — reemplaza el pendiente Lavalink:
   ...
   - [ ] **Subir el zip y repetir /play**: AHORA los logs mostrarán `metadatos fallo con sin cookies client=...` y luego intentos con `cookies:/tmp/...` — si aun así falla TODO, es que la IP está quemada incluso con cookies → toca `YT_PROXY` residencial (paso 2). Verificar también que `YT_COOKIES` esté seteada en el panel.
   ...
   - [ ] Con el nuevo zip: mirar la línea `[BOOT] DIAG YT_COOKIES` en logs → si dice NO seteada, la variable del panel no existe/mal escrita/no reinició; si dice presente pero formato raro, re-exportar; si Netscape OK y aún falla todo → IP quemada → YT_PROXY.
