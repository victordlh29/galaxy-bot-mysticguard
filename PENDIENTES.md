...
1d. **Anti-bot-check SIN proxy: PO Token provider bgutil integrado (23/08)** — opción 2 del plan contra la IP quemada (la opción 3, WARP/wireproxy, queda RESERVADA abajo):
   ...
   - [ ] **Subir `deploy-justrunmy-v11.zip`**: self-test con REINTENTOS (cada 500ms hasta deadline 15s) — si conecta pero el túnel no responde → "¿UDP saliente bloqueado?" (sentencia definitiva); si nunca abre puerto → "proceso murió". Además wireproxy ahora SIEMPRE loguea su salida (código o señal + último stderr) y tiene handler de 'error'. Testado local con servidor SOCKS5 simulado: inmediato OK / carrera de 2s OK (reintento) / puerto muerto 15s OK. Logs esperados: O `[WARP] proxy activo: YT_PROXY=socks5://127.0.0.1:25344` (¡WARP funcionando!) O `[WARP] handshake SOCKS5 falló: conectó al SOCKS5 pero el túnel no respondió en 15s...` → entonces sí, UDP bloqueado en justrunmy → YT_PROXY residencial pagado o correr en casa.
   - [ ] Tras WARP activo: verificar IP de salida distinta (la 152.53.157.177 debe desaparecer de las peticiones a YouTube) y que /play resuelva. El POT sigue corriendo: tokens + IP limpia es la combinación ideal.
   ...
   - [ ] **OPCIÓN 3 RESERVADA (plan B, no activada)**: Cloudflare WARP gratis como SOCKS5 local vía `wgcf register --accept-tos && wgcf generate` + binario `wireproxy` (WireGuard userspace, sin root) apuntando `YT_PROXY=socks5://127.0.0.1:25344`. Las IPs de WARP no están quemadas por YouTube. Solo montarlo si la opción 2 no basta.
